...
```json
{
  ...
  "narration_script": "此镜头的完整旁白（最终成品，可直接配音）。字数要与镜头时长匹配（5s≈18-20字，8s≈28-32字，10s≈35-40字）。语言有温度、有节奏感，能带动情绪。避免照本宣科，要像讲故事而非念说明书。",
...
1. **引入钩子**（8s）：直击痛点的问题或数据。情绪：共鸣/疑问。画面：城市冷调远景或孤立人物
2. **人物/背景**（8s）：介绍当事人——年龄、职业、家庭、当时处境。情绪：代入/担忧。画面：冷调家庭或职场场景
3. **问题/风险**（8s）：具体风险暴露，说清楚"不保会怎样"。情绪：紧张/沉重。画面：最冷调，阴云、医院、困境象征
4. **方案/解法**（10s）：港险具体方案——险种、保额、保费、优势。情绪：转折/希望。画面：光线开始变暖，阳光穿云
5. **结果/验证**（8s）：案例结局或实际效益，数据说话。情绪：安心/信任。画面：暖调，文件签署、家人重聚
6. **升华/行动**（8s）：情感收尾，留下思考。情绪：温暖/感召。画面：最暖，夕阳剪影或家庭温馨场景
